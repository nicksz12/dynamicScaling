...
ylabel('F_0')
xticks([1e-3,1e0,1e3])
title({'C. Steady state force','required for X = 1'})
...
hold on
hArea = area([1e-3,1e3],[60,60;60,60;60,60]','linestyle','none');
hArea(1).FaceColor = [1 1 1];
hArea(1).FaceAlpha = 0;
hArea(2).FaceColor = [0 0 0];
hArea(2).FaceAlpha = .05;
hArea(3).FaceColor = [0 0 0];
hArea(3).FaceAlpha = .1;
hold on
