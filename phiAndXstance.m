function [figHandle,figHandle2,figHandle3,figHandle4] = phiAndXstance(tPowerMin,tPowerMax,lPowerMin,lPowerMax,behaviorList,baseParams,nSamps)
...
%     zetaOfL = @(L) cOfL(L)./(2*sqrt(kOfL(L).*mOfL(L)));
    ...
    %damped).
%     LcritDamped = fzero(@(L)1-zetaOfL(L),[1e-6,1e-2]);
    Lunstable = fzero(@(L) kOfL(L),[1e-3,1]);

    phiLunstable = atan2d(cOfL(Lunstable).*(2*pi./t),kOfL(Lunstable)-mOfL(Lunstable).*(2*pi./t).^2);
    ...
    plot3(t,Lunstable+zeros(size(t)),phiLunstable,'k','linewidth',1)
    ...
    for i=1:5
        [tSim{i},xSim{i},~,~,~,~,~,Fapp{i},hFig] = simulateJointResponseStance(k0,c0,m0,s,a,Lletter(i),3,Tletter(i),15*Tletter(i),sp1,sp2);
        close(hFig);
    end
    ...
    for i=1:5
        ...
        cvec = autumn(3);
        %x sim is negative because of how 6, muscle would shorten. Consider:
...
%     keyboard
    ...
    plot3([10^tPowerMin,10^tPowerMax],Lunstable*[1,1],[180,180],'color',bndColor,'linewidth',lw,'linestyle',':')
    xlim([10^tPowerMin,10^tPowerMax])
    ylim([10^lPowerMin,10^lPowerMax])
    view(0,90)
    ...
    stimMultiplier = ones(5,1)*2.5;
    numCycles = [5;5;5;10;5];
    
    sp2 = [];
    
    for i=1:5
        sp1 = subplot(3,3,spInd(i));
        [tSim{i},xSim{i},~,~,~,~,~,Fapp{i},~] = simulateJointResponseStance(k0,c0,m0,s,a,Lletter(i),numCycles(i),Tletter(i),stimMultiplier(i)*Tletter(i),sp1,sp2);
        title(letterCell{i})
    ...
    for i=1:length(Lscale)
        ...
        sgtitle(sprintf('Relative force magnitude and phase shift in stance, L = %0.3f m',Lscale(i)))
        ...
        if i == LindToPlot
            ...
            title(sprintf('F. Relative force magnitude and phase shift in stance, L = %0.3f m',Lscale(i)),'FontSize',8,'FontWeight','normal')
            ...
            subplot(6,1,3:4)
            hold on
            plot3(t,Lscale(LindToPlot)+zeros(size(t)),phiLength,'-','linewidth',2,'color',[.5,.5,.5])
...
%             plot3(t,Lscale(LindToPlot)+zeros(size(t)),phiLength+.1,'k-','linewidth',2)
    ...
    for i=1:length(Tscale)
        ...
        sgtitle(sprintf('Relative force magnitude and phase shift in stance, T = %0.3f s',Tscale(i)))
        ...
        if i == TindToPlot
            ...
            grid on
            legend('elas.','grav.','visc.','iner.')
            title(sprintf('D. Relative force magnitude and phase shift in stance, T = %0.1f s',Tscale(i)),'FontSize',8,'FontWeight','normal')
            ...
            subplot(6,1,3:4)
            hold on
            plot3(Tscale(TindToPlot)+zeros(size(l)),l,phiFreq,'--','linewidth',2,'color',[.5,.5,.5])
            zlim([0,180])
            zticks(0:45:180)
