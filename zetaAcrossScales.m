...
cEmpMean = zeta.*(2*sqrt(kTotal.*Jemp));
cEmpUp = zetaUp.*(2*sqrt(kTotal.*Jemp));
cEmpDn = zetaDn.*(2*sqrt(kTotal.*Jemp));

h2 = figure;
errorbar(L,cEmpMean,cEmpMean-cEmpDn,cEmpMean-cEmpUp,'.','linewidth',lw)
hold on
plot(l,c)
title('Rotary damping scales with L^3 as predicted')
grid on
xlabel('L (m)')
ylabel('c_r (Nms)')
ax = gca;
...
ax.Position = [0.2750    0.1606    0.4500    0.7500];
h2.Position = [551.4000 396.2000 404.8000 246];
